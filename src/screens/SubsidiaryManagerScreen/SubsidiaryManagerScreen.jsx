...
const SubsidiaryManagerScreen = ({ location }) => {
  const [curBab, setCurTab] = useState(location.pathname === parentPaths.subsidiaryManager.main
    ? tabsData[0].path
    : location.pathname);
  const drawAddButton = () => {
      ...
      >
        {currentTab.button && (
          <Box alignSelf='flex-end'>
            <Button
              id='add-subsidiaryManager-button'
              color='primary'
              size='large'
              variant='contained'
              component={Link}
  ...
  return (
    <>
      ...
      <Switch>
        ...
        /> */}
        <Redirect exact from={`${parentPaths.subsidiaryManager.main}`} to={tabsData[0].path} />
      </Switch>
    </>
  );
};

SubsidiaryManagerScreen.propTypes = {
